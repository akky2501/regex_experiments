...
/*const std::set<StateID>& operator+=(std::set<StateID>& lhs, const std::set<StateID>& rhs){
	for (auto& it : rhs){
		lhs.insert(it);
	}
...
public:
	...
	void printState(void){
		int i = 0;
		std::cout << "NFA State---------------------" << std::endl;
		for (auto& it : states){
			std::cout << i << " : {" << std::endl;

			std::cout << "	end : " << ((i == countState() - 1) ? "true" : "false") << std::endl;

			std::cout << "	table : {" << std::endl;
			for (auto& t : it.table){
				std::cout << "		[ " ;
				if (t.first == EpsilonEvent) std::cout << "eps";
				else std::cout << "'" << t.first << "'";
...
private:
	...
	bool acceptImpl(StateID current, std::string::iterator it){

		if (current == states.size() - 1){
			if (it == send) return true;
			else return false;
		}
		
		bool res = false;
		
		for (auto &i : states[current].table){
			if (i.first == EpsilonEvent) res |= acceptImpl(i.second, it);
		}

		if (it != send){
			for (auto &i : states[current].table){
				if (i.first == *it || i.first == AnyEvent) res |= acceptImpl(i.second, it+1);
			}
		}
...
public:
	...
	void printState(void){
		...
		std::cout << "DFA State(Converter)---------------------" << std::endl;
		for (auto& it : states){
			std::cout << i << " : {" << std::endl;
			std::cout << "	nfa : ";
			for (auto& ns : it.nfa_state){
				std::cout << ns << ",";
			}
			std::cout << std::endl;

			std::cout << "	end : " << ((it.dfa_state.is_end) ? "true" : "false") << std::endl;

			std::cout << "	table : {" << std::endl;
			for (auto& t : it.dfa_state.table){
				std::cout << "		[ " << "'" << t.first << "'" << " -> " << t.second << " ]" << std::endl;
			}
			std::cout << "	}" << std::endl;
...
public:
    ...
    :lex(str_)
    {
		...
		convertNFA(root);
		//std::cout << "nfa_state:" << nfa.countState() << std::endl;
		nfa.printState();
...
int main(void){
	...
	while (1){
		std::cin >> str;
		if(str[0] == 'Q') break;
		std::cout << re.match(str) << std::endl;
	}
